/* Hashbill payment watcher — public RPCs only. No wallet connection. */
(function (global) {
  ...
  const STORAGE_UNLOCK = "hashbill_unlock";
  const STORAGE_CHECKOUT = "hashbill_checkout";
  const STORAGE_FROM = "hashbill_from";
  const STORAGE_PDFS = "hashbill_free_pdfs";
  ...
  function freePdfsUsed() {
    return Number(localStorage.getItem(STORAGE_PDFS) || "0") || 0;
  }

  function recordFreePdf() {
    localStorage.setItem(STORAGE_PDFS, String(freePdfsUsed() + 1));
  }
  ...
  async function checkTxHash(hash, expectedWei) {
    ...
    if (!sameAddr(tx.to, RECEIVE)) throw new Error("That transfer is not to the Hashbill receive address.");
  ...
  global.HashbillPay = {
    ...
    stopWatch: stopWatch,
    freePdfsUsed: freePdfsUsed,
    recordFreePdf: recordFreePdf,
    weiToEth: weiToEth,
